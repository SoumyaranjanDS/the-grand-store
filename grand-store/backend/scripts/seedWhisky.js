...
const filePath = path.join(__dirname, '../../frontend/public/grandstore_whisky_final_country_subcategory_brand.xlsx');
...
async function seed() {
  try {
    ...
    console.log("Connected to MongoDB for Whisky seeding.");
    ...
    if (!headers || !headers.includes('Product Name')) {
      console.error(`Unexpected header structure in Whisky file. Skipping.`, headers);
      process.exit(1);
    }
