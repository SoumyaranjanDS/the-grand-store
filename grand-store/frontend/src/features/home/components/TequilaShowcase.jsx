...
export default function TequilaShowcase({ onAdd, onWish, onCompare, compareItems }) {
  ...
  const tequilaProducts = products
    .filter((product) => (
      String(product.category || product.type || '').toLowerCase() === 'tequila'
    ))
    .slice(0, 5)
      ...
      <section className="section tequila-showcase home-product-editorial" id="tequila" ref={sectionRef}>
        <div className="shell">
          <div className="section-heading tequila-heading">
            <div>
              <p className="eyebrow">From the heart of agave country</p>
              <h2>Top Tequila</h2>
              <p className="section-intro">
                Reposado warmth, crystalline clarity and rare extra añejo—selected for the modern cabinet.
              </p>
            </div>
            <Link className="text-link arrow-link" to="/shop?category=Tequila">
        ...
        <div className="relative max-w-[1440px] mx-auto text-center px-6 sm:px-8 lg:px-7">
          <div className="relative border border-white/10 rounded-2xl py-8 bg-white/[0.01]">
            <div className="mb-8 relative">
              <h2 id="tequila-brands-title" className="m-0 font-serif text-[clamp(40px,3.8vw,62px)] font-medium tracking-[-0.02em] leading-[1.05] text-[#eee8dd] relative z-10">
                Top Tequila Brands
              </h2>
