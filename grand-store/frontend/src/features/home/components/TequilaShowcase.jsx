...
export default function TequilaShowcase({ onAdd, onWish, onCompare, compareItems }) {
  ...
  const tequilaProducts = products.filter((product) => {
    const cat = product.category || product.type
    return cat === 'Tequila' || cat === 'tequila'
  })
  const marqueeBrands = [...tequilaBrands, ...tequilaBrands]

  return (
    <>
      <section ref={sectionRef} className="relative py-8 md:py-10 border-t border-white/10 bg-[#0b0a08] bg-[radial-gradient(circle_at_26%_18%,rgba(102,153,102,0.08),transparent_34rem)] overflow-hidden" id="tequila">
