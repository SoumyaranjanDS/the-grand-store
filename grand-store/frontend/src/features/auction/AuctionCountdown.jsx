...
export default function AuctionCountdown({ endTime, now, compact = false }) {
  ...
  if (compact) return <time>{String(remaining.days).padStart(2, '0')}d {String(remaining.hours).padStart(2, '0')}h {String(remaining.minutes).padStart(2, '0')}m</time>

  return (
    <div className="auction-countdown" aria-label={`${remaining.days} days, ${remaining.hours} hours, ${remaining.minutes} minutes and ${remaining.seconds} seconds remaining`}>
      {[
        ['Days', remaining.days],
        ['Hrs', remaining.hours],
