...
export default function ProductPage({ onAdd, onWish, compareItems, onNotify }) {
  ...
  const [selectedOption, setSelectedOption] = useState(product?.options?.[0] ?? 'Pack of 1')
    ...
    <main className="min-h-screen bg-[#0a0907] pt-16 pb-12 px-4 md:px-8 lg:px-12 text-[#eee8dd] font-sans">
      ...
      <section className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 mb-16">
        ...
        <div className="flex flex-col justify-center">
          ...
          <div className="flex items-center gap-2 text-xs font-semibold tracking-widest uppercase mb-4">
            <span className="text-[#918a7f]">SKU: {String(product.id).substring(0, 10).toUpperCase()}</span>
            <span className="text-white/20">—</span>
            <span className={product.stock > 0 || product.stock === undefined ? "text-green-500" : "text-red-500"}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-y-6 gap-x-4 mb-8">
            {gridItems.slice(0, 6).map((item, idx) => (
              <div key={idx}>
                <div className="text-[10px] text-[#918a7f] uppercase tracking-widest font-semibold mb-1.5">{item.label}</div>
          ...
          {product.options && product.options.length > 1 && (
            <div className="mb-6">
              <label className="block text-[10px] text-[#918a7f] uppercase tracking-widest font-semibold mb-2">Options</label>
              <select 
              ...
              >
                {product.options.map((option) => <option className="bg-[#0a0907]" value={option} key={option}>{option}</option>)}
              </select>
            </div>
          )}
          ...
          <div className="flex items-center gap-6 text-[10px] uppercase tracking-widest font-semibold text-[#918a7f]">
            ...
            <button onClick={shareProduct} className="hover:text-[#eee8dd] transition-colors p-1" title="Copy Link"><Link2 size={24} /></button>
          </div>
      ...
      <section className="max-w-7xl mx-auto">
          ...
          <div className="md:col-span-9">
            <div className="max-w-3xl space-y-6 text-[#918a7f] text-sm leading-relaxed">
              ...
              {product.tastingNotes && product.tastingNotes.length > 0 && (
                <div className="mt-6">
                  <strong className="block text-[#eee8dd] mb-2 font-serif text-lg">Tasting Notes</strong>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-10 py-8 border-b border-white/10">
          <div className="md:col-span-3">
            <h3 className="text-xl font-serif text-[#eee8dd]">Additional Info</h3>
          </div>
          <div className="md:col-span-9">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              <div className="space-y-1">
                <div className="text-[10px] text-[#918a7f] uppercase tracking-widest font-semibold">Category</div>
                <div className="text-sm font-medium">{product.category || product.type || 'N/A'}</div>
              </div>
              <div className="space-y-1">
                <div className="text-[10px] text-[#918a7f] uppercase tracking-widest font-semibold">Brand</div>
                <div className="text-sm font-medium">{product.brand || product.name}</div>
              </div>
              <div className="space-y-1">
                <div className="text-[10px] text-[#918a7f] uppercase tracking-widest font-semibold">Origin</div>
                <div className="text-sm font-medium">{product.origin || 'N/A'}</div>
              </div>
              {detailEntries.map(([label, value]) => (
                <div className="space-y-1" key={label}>
                  <div className="text-[10px] text-[#918a7f] uppercase tracking-widest font-semibold">{label}</div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {relatedProducts.map((item) => (
              <ProductCard
                ...
                onAdd={onAdd}
                onWish={onWish}
                isCompared={compareItems && compareItems.some((productItem) => productItem.id === item.id)}
                key={item.id}
              />
            ))}
          </div>
