    ...
    <div className="flex flex-col gap-8 w-full max-w-7xl mx-auto">
      <section className="mb-2">
        <h1 className="text-[var(--color-ivory)] font-serif text-4xl mb-4">
          Auction <span className="font-script text-5xl text-[var(--color-gold)] font-normal ml-2 tracking-wide drop-shadow-[0_0_15px_rgba(212,175,55,0.3)]">Inventory</span>
        </h1>
      ...
      <section className="w-full bg-[#0a0a0a] border border-white/5 rounded-2xl p-4 md:p-8 shadow-2xl">
        <div className="pb-6 border-b border-white/[0.05] flex items-center justify-between">
          ...
          <button onClick={() => navigate('/vendor/auction-submit')} className="px-6 py-2.5 rounded-full bg-[var(--color-gold)]/10 text-[var(--color-gold)] border border-[var(--color-gold)]/30 font-semibold uppercase tracking-widest text-xs transition-all hover:bg-[var(--color-gold)] hover:text-black hover:shadow-[0_0_15px_rgba(212,175,55,0.4)]">
            + New Lot
          </button>
        </div>
              ...
              <tbody className="divide-y divide-white/5">
                {lots.map((lot) => (
                  <tr key={lot._id} className="hover:bg-white/[0.02] transition-colors group">
                    ...
                    <td className="py-5 px-5 text-[var(--color-gold)] font-sans font-semibold tracking-wide">R{lot.currentBid?.toLocaleString('en-ZA')}</td>
                    <td className="py-5 px-5">{getStatusBadge(lot.status)}</td>
                    <td className="py-5 px-5 text-right">
                      <button
                        onClick={() => navigate(`/auction/${lot._id || lot.id}`)}
                        className="px-4 py-1.5 rounded-full border border-[var(--color-gold)]/30 text-[var(--color-gold)] hover:bg-[var(--color-gold)]/10 transition-all text-[10px] uppercase tracking-widest"
                      >
                        View
                      </button>
                    </td>
