...
import { ShoppingBag, Package, MapPin, Search, Truck } from 'lucide-react';
...
export default function VendorOrders() {
  ...
  const filteredShipments = shipments.filter(shp => 
    (shp.shipmentId || shp._id).toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="flex flex-col gap-8 w-full max-w-7xl mx-auto pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-white/10 pb-6">
        <div>
          <h1 className="text-[var(--color-ivory)] font-serif text-4xl mb-2 flex items-center gap-4">
            ...
            Fulfillment <span className={`${goldTextClass} ml-2`} style={scriptFont}>Shipments</span>
          </h1>
          <p className="text-[var(--color-ivory-muted)] text-sm max-w-2xl font-light">
            Manage your outgoing shipments. Orders containing items from multiple vendors are split into individual shipments automatically.
          </p>
        </div>
      ...
      {loading ? (
        <div className="text-gold-gradient p-10 text-center">Loading your shipments...</div>
      ) : filteredShipments.length === 0 ? (
        <div className="text-center py-20 border border-white/5 rounded-3xl bg-white/[0.01]">
          <Truck size={48} className="mx-auto mb-4 text-[var(--color-ivory-muted)] opacity-20" />
          <p className="text-[var(--color-ivory-muted)] text-lg">No shipments found.</p>
        </div>
      ) : (
        <div className="space-y-6">
          {filteredShipments.map((shp) => (
            <div key={shp._id} className="bg-white/[0.02] border border-white/5 hover:border-white/10 rounded-2xl overflow-hidden transition-all">
              ...
              <div className="bg-black/40 px-6 py-4 flex flex-col md:flex-row justify-between items-start md:items-center border-b border-white/5 gap-4">
                <div className="flex flex-col md:flex-row gap-2 md:gap-8">
                  <div>
                    <div className="text-[10px] text-[var(--color-ivory-muted)] uppercase tracking-widest mb-1">Shipment ID</div>
                    <div className="text-sm text-gold-gradient font-bold">{shp.shipmentId || shp._id}</div>
                  </div>
                  <div>
                    ...
                    <div className="text-sm text-[var(--color-ivory)] font-serif">
                      {new Date(shp.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                    </div>
                  </div>
                  <div>
                    <div className="text-[10px] text-[var(--color-ivory-muted)] uppercase tracking-widest mb-1">Status</div>
                ...
                <div className="text-left md:text-right">
                  <div className="text-[10px] text-[var(--color-ivory-muted)] uppercase tracking-widest mb-1">Products Total</div>
                  <div className="text-xl font-serif text-gold-gradient">{formatCartPrice(shp.vendorTotal)}</div>
                </div>
              </div>
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div className="col-span-1 border-r border-white/5 pr-4">
                  ...
                  <div className="space-y-1">
                    <p className="text-sm font-bold text-[var(--color-ivory)]">{shp.customerName}</p>
                    <p className="text-xs text-[var(--color-ivory-muted)]">Courier: {shp.courierName}</p>
                  </div>
                  
                  {shp.deliveryAddress ? (
                    <div className="mt-4 text-sm text-[var(--color-ivory-muted)] leading-relaxed">
                      {shp.deliveryAddress.address}<br />
                      {shp.deliveryAddress.city}, {shp.deliveryAddress.postalCode}<br />
                  ...
                  {shp.trackingNumber && (
                    <div className="mt-4 p-3 bg-white/5 rounded border border-white/10">
                      <p className="text-xs text-[var(--color-ivory-muted)] uppercase tracking-widest mb-1">Tracking Number</p>
                      <p className="text-sm text-gold font-mono">{shp.trackingNumber}</p>
                    </div>
                  )}
                </div>
                ...
                <div className="col-span-1 md:col-span-2">
                  <h4 className="text-xs uppercase tracking-widest text-[var(--color-ivory-muted)] mb-4 flex items-center gap-2">
                    <Package size={14} className="text-gold-gradient" /> Products to Pack
                  </h4>
