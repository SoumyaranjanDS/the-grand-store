...
export default function BidderKycCard({ onNotify }) {
  ...
  const fetchBidderStatus = async () => {
    try {
      ...
      console.error('Error fetching bidder status:', err);
    } finally {
      setLoading(false);
    }
  };
      ...
      {isUnregistered && (
        <div className="space-y-4">
          <p className="text-xs text-white/70 leading-relaxed font-light">
            ...
            Choose between <strong>Standard Bidding (Free, R25,000 limit)</strong> or <strong>Premium VIP Bidding (R5,000 refundable guarantee deposit, R250,000+ limit)</strong>.
          </p>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
            ...
            <div className="bg-black/40 p-3 rounded-xl border border-white/5">
              ...
              <span className="text-[var(--color-gold)] font-mono font-bold">
                R{(profile.biddingLimit || 0).toLocaleString()}
              </span>
            </div>
            <div className="bg-black/40 p-3 rounded-xl border border-white/5">
              ...
              <span className={`font-semibold capitalize ${profile.bidderDepositStatus === 'paid' ? 'text-emerald-400' : profile.bidderDepositStatus === 'pending' ? 'text-amber-400' : 'text-white/40'}`}>
                {profile.bidderDepositStatus === 'paid' ? `R${(profile.bidderDepositAmount || 5000).toLocaleString()} (Paid)` : profile.bidderDepositStatus === 'pending' ? 'Awaiting Verification' : 'None Required'}
              </span>
            </div>
          </div>
            ...
            <div className="p-4 rounded-xl bg-gradient-to-r from-[#c9a35b]/10 via-[#c9a35b]/5 to-transparent border border-[#c9a35b]/30 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="space-y-1">
                ...
                <p className="text-[11px] text-white/60 leading-relaxed max-w-lg">
                  Want to bid on reserve, rare & high-value lots? Place an <strong>R5,000 security guarantee deposit</strong> to unlock bidding limits up to <strong>R250,000+</strong>. 
              ...
              <button
                type="button"
                onClick={() => setModalOpen(true)}
                className="py-2 px-4 rounded-lg bg-gold-gradient text-black font-bold uppercase tracking-wider text-[11px] hover:brightness-110 transition-all flex items-center justify-center gap-1.5 cursor-pointer whitespace-nowrap shrink-0 shadow-[0_0_12px_rgba(212,175,55,0.25)]"
              >
          ...
          {isVip && (
            <div className="p-3 bg-white/[0.02] border border-white/5 rounded-xl text-xs text-white/60 flex items-center justify-between">
              <span className="flex items-center gap-2">
                ...
                <span>You have unlocked full high-value bidding reserves across all Grand Store live auctions.</span>
              </span>
              <span className="text-[10px] text-emerald-400 font-semibold uppercase tracking-wider bg-emerald-500/10 px-2 py-0.5 rounded">
                Escrow Protected
              </span>
            </div>
          )}
