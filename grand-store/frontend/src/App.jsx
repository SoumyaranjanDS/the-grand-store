...
function App() {
  ...
  const addToCart = (product, quantity = 1, option = product.options?.[0] || 'Pack of 1') => {
    setCartItems((items) => {
      ...
      const existingItem = items.find((item) => (item.id || item._id) === prodId && item.option === option)
      if (existingItem) {
  ...
  const updateCartQuantity = (productId, option, quantity) => {
    if (quantity < 1) {
      setCartItems((items) => items.filter((item) => !((item.id || item._id) === productId && item.option === option)))
      return
    }
    setCartItems((items) => items.map((item) => (
      (item.id || item._id) === productId && item.option === option ? { ...item, quantity } : item
    )))
  }

  const removeFromCart = (product) => {
    const prodId = product.id || product._id;
    setCartItems((items) => items.filter((item) => !((item.id || item._id) === prodId && item.option === product.option)))
  ...
  const clearVendorCart = (vendorId) => {
    setCartItems((items) => items.filter((item) => (item.storeId || item.vendorId) !== vendorId))
  }
    ...
    <div className="app">
      ...
      {!isTradeRoute && !isDashboardRoute && (
        <>
          <Footer />
          {/* SocialRail removed */}
          <a className="whatsapp-float" href="https://wa.me/" aria-label="Chat with The Grand Store">
